...
* Configure firewall to allow incoming HTTPS requests from your local network:
  ...
  ```sh
  $ sudo ufw allow from 192.168.0.0/16 to any port 4001 comment 'allow RTL SSL from local network'
  $ sudo ufw status
  ```
...
Installation can take some time, and can hang on a single package for a long time.
If that happens, just give it some time.
